...
class TestA implements utest.ITest {
	...
	function testLayer() {
		final items:Array<GItem> = [
			...
			Rect(-100, -100, 100, 100, GFill.Solid(Red), GStroke.Stroke(Yellow, 10)),
		...
		// sSurface.addItems([Path(path, GFill.Solid(Blue), null)]);
		final svg = sSurface.render();

		// render to canvas

		final cSurface = new graphics.GSurfaceCanvas();
